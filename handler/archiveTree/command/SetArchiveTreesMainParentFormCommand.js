...
module.exports.setArchiveTreesMainParentForm = async (req, res) => {
  ...
  if (
    !isValidObjectId(archiveTreeId) ||
    !isValidObjectId(formSelected) ||
    archiveTreeId?.length < 7 ||
    formSelected?.length < 7
  )
    return res.send("none");
  else {
    const archiveTree = await ArchiveTreeModel.findByIdAndUpdate(
      archiveTreeId,
      {
        $set: {
          form: formSelected,
