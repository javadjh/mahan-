import { Button, Col, Form, Image, Input, Radio, Row, Select, Slider } from 'antd';
        ...
        <div style={{padding:30}}>
          ...
          <Form.Item name={"deviceId"}>
            <Select style={{width:"100%"}}>
              {devices.map(device=>(
                <Select.Option key={device.DeviceId} value={device.DeviceId}>{device.DeviceName}</Select.Option>
              ))}
            </Select>
          </Form.Item>
          <div>رزولوشن</div>
          ...
          <div style={{marginTop:50}}>
            <Row>
              <Col span={11}>
                <Button htmlType='submit' block>اسکن</Button>
              </Col>
              <Col hidden={!file} span={12} offset={1}>
                <Button onClick={()=>{
                  onScannedListener(file)
                }} block type='primary'>ارسال</Button>
              </Col>
            </Row>
          </div>
          </div>
