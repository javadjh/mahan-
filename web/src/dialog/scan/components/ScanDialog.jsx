...
const ScanDialog = ({onScannedListener})=>{
  ...
  const getData = async ()=>{
    const {data} = await axios.get("http://localhost:8080/api/Scanner/GetDevices",{
      headers:{
        ...
        <Form onFinish={scanDocument}>
          {devices.length>0?(
            <div className="flex-parent">
              <ScannerOptionsComponent onScannedListener={onScannedListener} devices={devices} file={file}/>
              <ShowDocumentScannedComponent isScanning={isScanning} blobImage={blobImage}/>
            </div>
          ):(
            <>
