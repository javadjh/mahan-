...
import {changeManualFileFormAction, getFileFormAction, setFileFormAction} from "../../stateManager/actions/FileAction";
...
const ElementsContextProvider = ({fileId,children,childrenListInput})=>{
    ...
    useEffect(()=>{
        ...
        return ()=>{
            setChildrenList([])
            dispatch(changeManualFileFormAction({
                form:{},
                filesForm:[]
            }))
        }
    ...
    const getData = async ()=>{
        if (fileId)
            await dispatch(getFileFormAction(fileId))
    }
