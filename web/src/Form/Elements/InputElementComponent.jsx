    ...
    switch (option.pattern) {
      ...
      case "melliCode":
        if (value.length !== 10) {
          ...
          if (!isPreview) data.isValid = false;
        } else {
          let L = value.length;
          ...
          setAlertText("شماره ملی صحیح نمیباشد");
          if (!isPreview) data.isValid = isValid;
        }
        break;
      case "phoneNumber":
        if (value.length !== 11) {
          ...
          if (!isPreview) data.isValid = true;
        }
        break;
      case "email":
  ...
  return (
    <>
      <div className="form-group">
        ...
        <input
          type={option.pattern === "number" ? "number" : "text"}
          name={"input"}
          disabled={isDisable}
          value={data.answer}
          onChange={(e) => {
            if (
              option.pattern === "melliCode" ||
              option.pattern === "email" ||
              option.pattern === "phoneNumber"
            ) {
