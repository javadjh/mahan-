...
const UpsertFormContextProvider = ({ children }) => {
  ...
  const addNewData = async (type, inputType) => {
    ...
    let data = {
      sortNumber:
        childrenListCopy.length <= 0
          ? 1
          : childrenListCopy[childrenListCopy.length - 1].sortNumber + 1,
      uiId: uuidv4(),
      type,
      isRequired: false,
      inputType,
  ...
  const onUpOrDownHandle = async (uiId, isUp) => {
    ...
    if (childrenListCopy[index - 1])
      childrenListCopy[index - 1].sortNumber = isUp
        ? childrenListCopy[index].sortNumber - 1
        : childrenListCopy[index].sortNumber + 1;
    ...
    childrenListCopy[index].sortNumber = isUp
      ? childrenListCopy[index].sortNumber + 2
      : childrenListCopy[index].sortNumber - 2;
