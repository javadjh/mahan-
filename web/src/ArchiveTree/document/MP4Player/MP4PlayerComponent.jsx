        ...
        <>
            <div>
                <div className={"row mx-2"}>
                    <i className={"mdi mdi-play custom-cursor"} style={{fontSize:20}} onClick={()=>{
                        setPlaying(true)
                    }}/>
                    <i className={"mdi mdi-pause mx-1 custom-cursor"} style={{fontSize:20}} onClick={()=>{
                        setPlaying(false)
                    }}/>
                    <i className={"mdi mdi-volume-mute mx-1 custom-cursor"} style={{fontSize:20}} onClick={()=>{
                        setIsMute(true)
                    }}/>
                    <i className={"mdi mdi-volume-high mx-1 custom-cursor"} style={{fontSize:20}} onClick={()=>{
                        setIsMute(false)
                    }}/>
                </div>
                ...
                <div className={"row"}>
                    <div className={"col-lg-6"}>
                        <div className={"my-2 mx-4"} style={{display:"flex",justifyContent:"center",alignItems:"center"}}>
                    <span onClick={()=>{
                        if(progress<=endSecond || endSecond===0)
                            setStartSecond(progress)
                    }} className={`custom-cursor mx-5 btn btn-${startSecond>0?"success":"dark"}`}>
                            زمان شروع {fancyTimeFormat(startSecond)}
                        </span>
                            <span onClick={()=>{
                                if(progress>=startSecond)
                                    setEndSecond(progress)
                            }} className={`custom-cursor mx-5 btn btn-${endSecond>0?"success":"dark"}`}>
                    ...
                    {startSecond>0 && endSecond>0?(
                        <div className={"col-lg-6 mt-2"}>
                            <div className={"row mx-1"}>
                                <input className={"form-control col-lg-10"} onChange={(e)=>{
                                    setFlatDescription(e.target.value)
                                }} placeholder={"توضیحات"}/>
                                <div className={"col-lg-2"}>
                                    <button className={"btn btn-success btn-block"} onClick={async ()=>{
                                        addNewFlagHandle(startSecond,endSecond,flatDescription).then(isSuccess=>{
                                            if(isSuccess){
                                                setFlatDescription('')
                    ...
                    <div className="table-responsive mb-0" data-pattern="priority-columns">
                        <table id="tech-companies-1" className="table table-striped">
                            ...
                            {singleDocumentState.videoFlags.map((f,index)=>(
                                <tr  style={progress>=f.startSecond && progress<=f.endSecond?
                                    {backgroundColor:"#65af43",color:"white"}:{}}>
                                    <th>{index+1}</th>
                                    <td >
                                        <span style={{color:"royalblue",cursor:"pointer"}} onClick={()=>{
                                    ...
                                    <td>
                                        ...
                                        }} className={"mdi mdi-close-circle-outline mx-2 mt-2"} style={{color:`${progress>f.startSecond && progress<f.endSecond?"white":"red"}`,fontSize:14}}/>
                                    </td>
