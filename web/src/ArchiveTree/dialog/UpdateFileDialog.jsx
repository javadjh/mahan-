...
import {
    ...
    setManualSingleFileArchive, updateFileAction
...
const UpdateFileDialog = ({fileId})=>{
    ...
    return(
        <Fragment>
            <div className="modal fade" id="updateFileDialog" tabIndex="-1" role="dialog"
                        ...
                        <div className={"col-lg-12 card"}>
                            {singleForAddFile._id?(
                                <span className={"row "}>
                                    ...
                                    {singleForAddFile.archiveTreeId.archive.availablePattern==="one" ||singleForAddFile.archiveTreeId.archive.availablePattern==="two" ||
                                    singleForAddFile.archiveTreeId.archive.availablePattern==="five"?(
                                        <div className="col-lg-4 mt-4" dir={"ltr"}>
                                                    <span className={"row"}>
                                                        {singleForAddFile.archiveTreeId.archive.availablePattern==="five"?null:(
                                                            <span className={"mt-2 mr-2 ml-1"}>{singleForAddFile.archiveTreeId.archive.availablePattern==="two"?
                                                                `${singleForAddFile.archiveTreeId.archive.firstStringOfPattern}-${
                                                                    moment1(new Date(), 'YYYY/MM/DD').locale('fa').format('YYYYMMDD')
                                                                }`:`${singleForAddFile.archiveTreeId.archive.firstStringOfPattern}-`}</span>
                                                        )}
                                                        <input type="text"
                                                               name={"fileCode"}
                                                               value={fileCode}
                                                               ...
                                                               className="form-control col-lg-8 mx-2" id="validationCustom04"
                                                               placeholder="شماره پرونده" required/>
                                                        {formValidator.current.message("fileCode",fileCode,
                                                            singleForAddFile.archiveTreeId.archive.availablePattern==="one" || singleForAddFile.archiveTreeId.archive.availablePattern==="two"
                                                            || singleForAddFile.archiveTreeId.archive.availablePattern==="four"?"required|min:3|max:20":"min:3|max:20")}
                                                    </span>
                                        </div>
                                    ):null}
                                    </span>
                            ...
                            <div className={"row mt-2"}>
                                    ...
                                    {singleForAddFile._id?(
                                        <>
                                            {singleForAddFile.archiveTreeId.lang==="fa"?(
                                                <div>
                                                    <label htmlFor="validationCustom04">تاریخ پرونده{moment1(singleForAddFile.fileDateMiladi, 'YYYY/MM/DD').locale('fa').format('YYYY/MM/DD')}</label>
                                                    <PersianDatePickerComponent onSelect={(momentDate)=>{
                                                ...
                                                <div>
                                                    <label htmlFor="validationCustom04">تاریخ پرونده(میلادی)</label>
                                                    <PersianDatePickerComponent  onSelect={(momentDate)=>{
                                                        setFaDate(momentDate + "")
                                                    }}/>
                                                </div>
                                ...
                                <div className="form-group col-lg-3">
                                    ...
                                    <select className="custom-select" onChange={(e)=>{
                                        ...
                                        <option value={"جاری"} selected={fileStatus==="جاری"} name={"جاری"}>جاری</option>
                                        <option value={"نیمه جاری"} selected={fileStatus==="نیمه جاری"} name={"نیمه جاری"}>نیمه جاری</option>
                                        <option value={"مختومه"} selected={fileStatus==="مختومه"} name={"مختومه"}>مختومه</option>
                                    </select>
                                ...
                                <div className="form-group col-lg-3">
                                    ...
                                    <select className="custom-select" onChange={(e)=>{
                                        ...
                                        <option value={"عادی"} selected={type==="عادی"} name={"عادی"}>عادی</option>
                                        <option value={"محرمانه"} selected={type==="محرمانه"} name={"محرمانه"}>محرمانه</option>
                                        <option value={"به کل محرمانه"} selected={type==="به کل محرمانه"} name={"به کل محرمانه"}>به کل محرمانه</option>
                                    </select>
                            ...
                            <div className={"row"}>
                                ...
                                <button className="btn btn-success col-lg-3" onClick={sendData}>ثبت پرونده</button>
                            </div>
                        </div>
                    </div>
                </div>
