...
import {getArchiveTreesAction, setManualArchiveTreesAction} from "../../stateManager/actions/ArchiveTreeAction";
...
const ArchiveTreeLineRoot = ({history})=>{
    ...
    const {archiveId,setArchiveId,isShowAddFile,setIsShowAddFile,setSingleForAddFile,singleForAddFile} = useContext(ArchiveTreeLineContext)
    ...
    },[archiveId,mainParent,pageId,reloadMainParentArchiveTree,isShowAddFile,searchValue])
    ...
    const getArchiveTrees = async ()=>{
        await dispatch(getArchiveTreesAction(mainParent.length>0?false:true,mainParent.length>0?mainParent[mainParent.length-1]._id:undefined,archiveId))
        if(mainParent.length>0)
            await dispatch(archiveTreesFilesAction(
    ...
    return(
        <>
            <ArchiveTreeSettingDialog inTree={false} currentArchiveTree={singleForAddFile}/>
                ...
                <div className={"row"}>
                    <div className={`col-lg-${isHideArchive?"0":"3"}`}>
                        <div hidden={isHideArchive}>
                            {usersArchives.map(u=>(
                                <div className={"card p-3 mb-2"} style={{backgroundColor:localStorage.getItem("archive")===u.archiveId._id?"#48B116":"#ffffff"}} >
                                    <div style={{display:"flex",justifyContent:"space-between"}}>
                                        <h5 className={"m-0 custom-cursor"} onClick={ async ()=>{
                                        ...
                                        }} style={{color:localStorage.getItem("archive")===u.archiveId._id?"#ffffff":"#000000"}}>{u.archiveId.title}</h5>
                                        <i className={"mdi mdi-settings custom-cursor"}  onClick={ async ()=>{
                                            console.log(u.archiveId._id)
                                            await dispatch(getSingleArchiveAction(u.archiveId._id))
                                        }} style={{color:localStorage.getItem("archive")===u.archiveId._id?"#ffffff":"orange",fontSize:19}}/>
                                    </div>
                                    <span style={{color:localStorage.getItem("archive")===u.archiveId._id?"#ffffff":"#000000"}}>{u.archiveId.description}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                                    ...
                                    <div hidden={isShowAddFile}>
                                        ...
                                        <>
                                            ...
                                            <div className={"row"}>
                                                {mainParent.length>0?(
                                                    <>
                                                         <span style={{cursor:"pointer"}} className={"px-2"} onClick={()=>{
                                                             setIsShowAddFile(false)
                                                             let mainParentCopy = [...mainParent]
                                                             mainParentCopy = mainParentCopy.slice(0,mainParentCopy.length-1)
                                                             setMainParent(mainParentCopy)
                                                         }}><i className={"mdi mdi-arrow-right-thick"}/></span>
                                                         {mainParent.map((m,index)=>(
                                                             <span style={{cursor:"pointer",marginBottom:10}} onClick={()=>{
                                                                 let mainParentCopy = [...mainParent]
                                                                 mainParentCopy = mainParentCopy.slice(0,index+1)
                                                                 setMainParent(mainParentCopy)
                                                             }}> {m.title} / </span>
                                                         ))}
                                                    </>
                                            ...
                                            <ArchiveTreeLineArchiveTable setMainParent={setMainParent} mainParent={mainParent} archives={archiveTrees}/>
                                        </>
                                    </div>
                                </div>
                                {isShowAddFile?(
                                            ...
                                            <>
                                                <div className={"card"}>
                                                    <div className={"card-body"}>
                                                        <h5>پرونده های بایگانی <span style={{color:"green"}}>( {mainParent[mainParent.length-1].title} )</span></h5>
                                                        <div style={{display:"flex",justifyContent:"space-between",marginBottom:10}}>
                                                            <div>
                                                                <button className={"btn btn-success "} onClick={()=>{
                                                                    setSingleForAddFile(mainParent[mainParent.length-1])
                                                                    setIsShowAddFile(true)
                                                                }}>افزودن پرونده</button>

                                                                <button className={"btn btn-light  mx-2"}>تعداد کل : {archiveTreesFiles.total}</button>
                                                            </div>
                                                            <input type="text"
                                                                   onChange={(e)=>{
                                                                        setSearchValue(e.target.value)
                                                                   }}
                                                                   className="form-control col-lg-5  " id="validationCustom05"
                                                                   placeholder="عنوان پرونده را وارد کنید..." required/>
                                                        </div>
                                                        {archiveTreesFiles.files.length>0?(
                                                            <span>
                                                                <ArchiveTreeLineFileTable history={history}
                                                                                          hasForm={true}
                                                                                          archiveId={archiveId}
                                                                                          files={archiveTreesFiles.files}/>
                                                                <PagingComponent eachPerPage={archiveTreesFiles.eachPerPage} handlePaging={(page)=>{
                                                                    setPageId(page)
                                                                }} pageId={archiveTreesFiles.pageId} total={archiveTreesFiles.total}/>
                                                            </span>
                                                            ):(
                                                                <>
                                                                    <hr/>
