import {
    deletePersonService,
    getPeopleService, insertPeopleFormExcelService,
    insertPersonService,
    updatePersonService
...
export const insertPersonAction = (person)=>{
    return async (dispatch) => {
        ...
        if(status===200){
            ...
            await dispatch(getPeopleAction({pageId: 1,eachPerPage:12,searchValue:''}))
            window.$('#upsertPersonDialog').modal('hide')
        }
    }
}
...
export const insertPeopleFormExcelAction = (file)=>{
    return async (dispatch) => {
        ...
        if(status===200){
            ...
            await dispatch(getPeopleAction({pageId: 1,eachPerPage:12,searchValue:''}))
            window.$('#insertPeopleFromExcelDialog').modal('hide')
        }
    }
}
