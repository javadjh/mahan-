import {
    addNewNoteForDocumentService, addVideoFlagService, changeDocumentFileService,
    changeDocumentsNameService,
    deleteDocumentService,
    deleteGroupDocumentsService, getDeActivateDocumentsService,
    getDocumentService,
    getDocumentsService,
    groupAddNoteDocumentService,
    removeNoteFromDocumentService, removeVideoFlagService, reportDamageDocumentService, restoreDocumentService,
    setDocumentsFileService
} from "../../service/DocumentService";
import {doneToast, infoToast} from "../../utility/ShowToast";
import {getFileLogsAction} from "./LogsAction";
import {getLibraryAction, getLibraryShelfDocumentsAction} from "./LibraryAction";
...
export const deleteDocumentAction = (documentId,fileId)=>{
    return async (dispatch)=>{
        ...
        if(status===200){
            ...
            await dispatch(getDocumentsAction({
                pageId:1,
                eachPerPage:12,
                searchValue:"",
                fileId
...
export const addNewNoteForDocumentAction = (note,fileId)=>{
    return async (dispatch)=>{
        ...
        if(status===200){
            await dispatch(getFileLogsAction({
                pageId:1,
                eachPerPage:12,
                searchValue:'',
                fileId
...
export const removeNoteFromDocumentAction = (documentId,id,fileId)=>{
    return async (dispatch)=>{
        const {status,data} = await removeNoteFromDocumentService(documentId,id)
        if(status===200){
            await dispatch(getFileLogsAction({
                pageId:1,
                eachPerPage:12,
...
export const changeDocumentFileAction = (reqData)=>{
    return async (dispatch)=>{
        ...
        if(status===200){
            ...
            await dispatch(saveDocumentsInfoAction({
                origin:undefined,
                documents:[]
            },null,true))
        }
...
export const restoreDocumentAction = (id,fileId)=>{
    return async (dispatch)=>{
        ...
        if(status===200){
            ...
            await dispatch(getDeActivateDocumentsAction({
                pageId:1,
                eachPerPage:12,
                fileId
            }))
