import {
    changeArchiveTreesNameService, deleteArchiveTreeService, getArchiveTreesService,
    insertArchiveTreeService, mainArchiveTreesFormService, searchArchivesTreesService,
    searchArchiveTreesService, settingArchiveTreesService
...
export const insertArchiveTreeAction = (archiveTree)=>{
    return async (dispatch)=>{
        ...
        if(status===200){
            await dispatch({type:"INIT_RELOAD_MAIN_PARENT_ARCHIVE_TREE",payload: {mainParent:data.mainParent}})
...
export const changeArchiveTreesNameAction = (id,archiveTreesTitle)=> {
    return async (dispatch) => {
        const {data, status} = await changeArchiveTreesNameService(id, archiveTreesTitle)
        if (status === 200) {
            await dispatch({type: "INIT_RELOAD_MAIN_PARENT_ARCHIVE_TREE", payload: {mainParent: data.mainParent}})
            window.$('#addTreeDialog').modal('hide')
...
export const mainArchiveTreesFormAction = (archiveTreeId,bodyData)=>{
    return async (dispatch)=>{
        const {status,data} = await mainArchiveTreesFormService(archiveTreeId,bodyData)
        if(status===200){
            await dispatch(getArchiveTreesAction(true,undefined,localStorage.getItem("archive")))
            await dispatch({type:"INIT_RELOAD_MAIN_PARENT_ARCHIVE_TREE",payload: {date:Date.now()}})
